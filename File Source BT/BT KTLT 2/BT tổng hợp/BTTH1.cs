...
namespace BT_tong_hop_1
{
    class Program
    {
        public struct sinhVien
        {
            ...
            {
                return $"{maSV}-{hoTen}-{ngaySinh}-{diemTBTN}-{bangDiemTB}";
            }
        }
        ...
        //Nhap diem TB cho SV
        static void NhapDTB(sinhVien sv)
        {
            for (int i = 0; i < sv.bangDiemTB.GetLength(0); i++)
            {
                Console.WriteLine($"Nhap diem hoc ky {i + 1}: ");
                for (int j = 0; j < sv.bangDiemTB.GetLength(1); j++)
                {
                    Console.WriteLine($"Nhap diem hoc ky {j + 1}: ");
                    double.TryParse(Console.ReadLine(), out sv.bangDiemTB[i, j]);
                }
                Console.WriteLine();
            }
        }
